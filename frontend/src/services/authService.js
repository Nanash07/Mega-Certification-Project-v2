...
export async function login({ username, password }) {
    try {
        const { data } = await api.post(`${BASE_URL}/login`, { username, password });
        return data;
    } catch (err) {
...
export async function resetPassword({ token, newPassword }) {
    try {
        const { data } = await api.post(`${BASE_URL}/reset-password`, { token, newPassword });
        return data;
    } catch (err) {
...
export async function validateResetToken(token) {
    try {
        const { data } = await api.get(`${BASE_URL}/reset-password/validate`, { params: { token } });
...
export async function changePasswordFirstLogin({ username, newPassword }) {
    try {
        const { data } = await api.post(`${BASE_URL}/change-password-first-login`, { username, newPassword });
        return data;
    } catch (err) {
