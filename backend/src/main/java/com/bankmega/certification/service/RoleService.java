...
public class RoleService {
    ...
    public Page<RoleResponse> getPage(String q, Pageable pageable) {
        if (q == null || q.isBlank()) {
            return roleRepo.findAll(Objects.requireNonNull(pageable)).map(this::toResp);
        }
        return roleRepo.findAll((root, cq, cb) -> {
            String like = "%" + q.trim().toLowerCase() + "%";
            return cb.like(cb.lower(root.get("name")), like);
        }, Objects.requireNonNull(pageable)).map(this::toResp);
    }

    public RoleResponse getById(Long id) {
        Role r = roleRepo.findById(Objects.requireNonNull(id))
    ...
    public RoleResponse create(RoleRequest dto) {
        ...
        Role saved = roleRepo.save(Objects.requireNonNull(Role.builder()
                .name(name)
                .build()));
        return toResp(saved);
    }
    ...
    public void delete(Long id) {
        ...
        roleRepo.delete(Objects.requireNonNull(r));
    }
